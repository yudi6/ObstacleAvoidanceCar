...
class ObstacleAvoidanceCar():
    ...
    def _get_handle(self):
        # 左轮的句柄
        _, self.left_motor = vrep.simxGetObjectHandle(self.clientId, "Pioneer_p3dx_leftMotor",
                                                      vrep.simx_opmode_oneshot_wait)
        # 右轮的句柄
        _, self.right_motor = vrep.simxGetObjectHandle(self.clientId, "Pioneer_p3dx_rightMotor",
                                                       vrep.simx_opmode_oneshot_wait)
        # 视觉传感器句柄
    ...
    def _get_image(self):
        _, resolution, image = vrep.simxGetVisionSensorImage(self.clientId, self.vision_sensor, 0,
    ...
    def _run(self):
        ...
        while True:
            ...
            contours_speed, cnt_speed = cv2.findContours(binary_speed.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            #   黑色块的比重
